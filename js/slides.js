...
var PERMANENT_URL_PREFIX = 'http://test.babux.fr/slide/';
...
//Moi
function goToSlide(nbSlide) {
  for (var i = curSlide; i < nbSlide; i++) {
	  if (nbSlide < slideEls.length) {
	    curSlide++;
	    window.setTimeout(function() {
	    updateSlides();},100);
	  }
...
//Moi
function deleteSlide(nbSlide) {
  if (curSlide > 0) {
    curSlide--;
	slideEls.length--;
    updateSlides();
    
    //On rafraîchi le nombre de slide
    window.setTimeout(function() {
